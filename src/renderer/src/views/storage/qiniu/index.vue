<template>
  <div class="page-storage">
    <header>
      <h1>{{ t('views.storage.qiniu.title') }}</h1>
      <div class="controls">
        <n-space>
          <n-tooltip>
            <template #trigger>
          ...
          <n-button v-permission="['storage:qiniu:add']" type="primary" @click="handleUpload">
            <template #icon>
              <n-icon>
                <Upload />
              </n-icon>
...
const columns = computed<DataTableColumn[]>(() => [
  ...
  },
  {
    ...
    render(row: any) {
      ...
      const hasDelete =
        permissions.includes('storage:qiniu:delete') || permissions.includes('*:*:*');
...
const handleActionSelect = (key: string, row: any) => {
  if (key === 'copy') {
    ...
    dialog.warning({
      title: t('views.txt.confirmDelete'),
      content: t('views.txt.deleteFileTip'),
...
const handleDelete = async (key: string) => {
  try {
    await deleteFile({ key, driver: 'qiniu' });
    message.success(t('views.txt.deleteSuccess'));
    fetchFiles();
  } catch (error) {
    message.error(t('views.txt.deleteFailed'));
  }
};

const handleCopyLink = (row: any) => {
  // For Qiniu with anti-leech, we must use the signed URL returned by backend
  // The backend generates this using the configured domain and keys
  const url = row.url;

  if (isSupported.value) {
    copy(url);
    message.success(t('views.txt.linkCopied'));
  } else {
    message.error(t('views.txt.clipboardNotSupported'));
  }
};
