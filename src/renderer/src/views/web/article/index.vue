<template>
  <div class="page-article">
    <header>
      <h1>{{ t('views.web.article.title') }}</h1>
      <div class="controls">
        <n-space>
          <n-tooltip>
            <template #trigger>
          ...
          <n-button @click="handleCategoryManage">
            {{ t('views.web.article.category.title') }}
          </n-button>
          <n-input-group>
            <n-input
      ...
      >
        ...
        >
          <n-form-item path="category_id">
            <template #label>
              <LabelWithTooltip
                :label="t('views.web.article.form.category.label')"
                :tooltip="t('views.web.article.form.category.tooltip')"
              />
            </template>
            <n-select
              v-model:value="formData.category_id"
              :options="categoryOptions"
              :placeholder="t('views.web.article.form.category.placeholder')"
              clearable
            />
          </n-form-item>
          <n-form-item path="title">
            <template #label>
              <LabelWithTooltip
                :label="t('views.web.article.form.title.label')"
                :tooltip="t('views.web.article.form.title.tooltip')"
              />
            </template>
            <n-input
              v-model:value="formData.title"
              :placeholder="t('views.web.article.form.title.placeholder')"
            />
          </n-form-item>
          <n-form-item path="permalink">
            <template #label>
              <LabelWithTooltip
                :label="t('views.web.article.form.permalink.label')"
                :tooltip="t('views.web.article.form.permalink.tooltip')"
              />
            </template>
            <n-input
              v-model:value="formData.permalink"
              :placeholder="t('views.web.article.form.permalink.placeholder')"
            />
          </n-form-item>
          <n-form-item path="description">
            <template #label>
              <LabelWithTooltip
                :label="t('views.web.article.form.description.label')"
                :tooltip="t('views.web.article.form.description.tooltip')"
              />
            </template>
            <n-input
              v-model:value="formData.description"
              :placeholder="t('views.web.article.form.description.placeholder')"
            />
          </n-form-item>
          <n-form-item path="content">
            <template #label>
              <LabelWithTooltip
                :label="t('views.web.article.form.content.label')"
                :tooltip="t('views.web.article.form.content.tooltip')"
              />
            </template>
            <EditorComponent v-model="formData.content" page-type="article"></EditorComponent>
          </n-form-item>
        </n-form>
...
const dataList = ref<ArticleModel[]>([]);
...
interface FormData extends Omit<ArticleModel, 'id' | 'created_at' | 'updated_at' | 'category'> {
  id: number;
...
const rules: FormRules = {
  title: {
    required: true,
    message: t('views.web.article.form.title.rules.required'),
    trigger: 'blur'
  },
  category_id: {
    type: 'number',
    required: true,
    message: t('views.web.article.form.category.rules.required'),
    trigger: ['blur', 'change']
  },
  permalink: {
    required: true,
    ...
    validator: async (_: any, value: string) => {
      if (!value) {
        return Promise.reject(Error(t('views.web.article.form.permalink.rules.required')));
      }
      if (!/^[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?$/.test(value)) {
        return Promise.reject(Error(t('views.web.article.form.permalink.rules.pattern')));
      }
      try {
        const isUnique = await checkArticlePermalink({ permalink: value, id: formData.id });
        if (!isUnique) {
          return Promise.reject(Error(t('views.web.article.form.permalink.rules.unique')));
        }
      } catch (error) {
...
const columns = computed<DataTableColumn<ArticleModel>[]>(() => [
  { title: t('views.web.article.form.title.label'), key: 'title' },
  { title: t('views.web.article.form.permalink.label'), key: 'permalink' },
  {
    title: t('views.web.article.category.title'),
    key: 'category_id',
    render(row) {
...
const fetchCategoryList = async () => {
  try {
    ...
    if (res) {
      categoryOptions.value = res.map((item: ArticleCategoryModel) => ({
        label: item.name,
        value: item.id
      }));
    }
...
const handleEdit = (row: ArticleModel) => {
  isEditing.value = true;
  formData.id = row.id;
  formData.title = row.title;
  formData.permalink = row.permalink;
  formData.description = row.description;
  // Replace asset src with absolute url if needed for preview, similar to news
  // But for now assuming direct usage or similar handling
...
const handleDelete = (row: ArticleModel) => {
  dialog.warning({
    title: t('global.txt.confirmDelete'),
    content: t('views.web.article.deleteTip', { title: row.title }),
